...
def getForecast(locations, user, phone, apikey):
    ...
    for location in locations: 
        ...
        if (weather!=""):
            msg2 = f"\nToday there will be an occurrence of {weather} around {location} area, with the condition of {condition}, around {time}."

        else:
            msg2 = f"\nNo major weather update for today for {location}."

        all_messages += msg2

    send_whatsapp(phone, apikey, msg1+all_messages+" Please prepare for your day accordingly. Have a wonderfull day!")
